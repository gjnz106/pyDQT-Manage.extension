...
"""
Check Update - DQT
Tells the user whether the installed pyDQT-Manage is up to date or a newer
...
__doc__ = "Check whether your pyDQT-Manage is the latest version (compares with GitHub)."
...
REPO = "gjnz106/pyDQT-Manage.extension"
DEFAULT_BRANCH = "main"
...
def main():
    ...
    if local is None:
        forms.alert(
            "Cannot read the local version - pyDQT-Manage does not look "
            "like a git install.\n\nTo enable one-click updates, install "
            "pyDQT-Manage through pyRevit > Extension Manager using the Git "
            "URL:\nhttps://github.com/{}.git".format(REPO),
            title="pyDQT-Manage - Check Update")
        return
    ...
    if local.startswith(remote) or remote.startswith(local):
        ...
        forms.alert(
            "A new version of pyDQT-Manage is available!\n\n"
            "Installed version: {}\n"
            "Latest on GitHub:  {}\n\n"
            "How to update:\n"
